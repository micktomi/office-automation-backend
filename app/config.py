...
def get_settings() -> Settings:
    ...
    return Settings(
        ...
        database_url=os.getenv("DATABASE_URL", "sqlite:///./office_agent.db").strip(),
        ...
        sms_from_number=os.getenv("SMS_FROM_NUMBER", "").strip(),
        twilio_account_sid=os.getenv("TWILIO_ACCOUNT_SID", "").strip(),
        twilio_auth_token=os.getenv("TWILIO_AUTH_TOKEN", "").strip(),
        twilio_messaging_service_sid=os.getenv("TWILIO_MESSAGING_SERVICE_SID", "").strip(),
        whatsapp_gateway_url=os.getenv("WHATSAPP_GATEWAY_URL", "http://localhost:3400/send-message").strip(),
        ...
        google_credentials_file=str(
            (backend_root / os.getenv("GOOGLE_CREDENTIALS_FILE", "client_secret.json").strip()).resolve()
        ),
        google_token_file=str((backend_root / os.getenv("GOOGLE_TOKEN_FILE", "token.json").strip()).resolve()),
        google_scopes=(
